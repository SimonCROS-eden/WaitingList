...
<div class="container">
    <div class="pt-5">
        <div class="d-flex justify-content-between mb-5">
            <h1 class="">Étiquettes</h1>
            <div class="d-flex align-items-center">
                <form class="d-flex align-items-center" method="POST" action="/tag">
                    @csrf

                    <input id="name" type="text" value="{{ old('name') }}" class="form-control @error('name') is-invalid @enderror" name="name" required autocomplete="name" placeholder="Nom étiquette">
                    @error('name')
                    ...
                    <div class="mr-1 mb-2 px-3 py-1 badge text-white" style="display:none; border: solid 1px {{ $tag->color }}">
                        <div class="d-flex align-items-center">
                            ...
                            <form action="/tag/{{ $tag->id }}" method="post">
                                @csrf
                                @method('DELETE')
                                <button class="input-button text-danger" type="submit">Supprimer</button>
                            </form>
                        </div>
                    </div>
                </div>
