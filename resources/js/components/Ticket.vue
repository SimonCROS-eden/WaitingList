<template>
    <div :data-id="ticket.id">
        <div class="d-flex justify-content-between mb-3">
            ...
            <div class="d-flex flex-row-reverse align-self-center">
                ...
                <template v-else-if="isOwner()">
                    <form class="ml-2" @submit.prevent="onSubmit" :action="'/ticket/' + ticket.id + '/end'" method="post">
                        <input type="hidden" name="_token" :value="csrf">
                        <button class="btn btn-primary" type="submit">
                            Finir
                        </button>
                    </form>
                    <form class="ml-2" @submit.prevent="onSubmit" :action="'/ticket/' + ticket.id + '/renew'" method="post">
                        <input type="hidden" name="_token" :value="csrf">
                        <button class="btn btn-primary" type="submit">
                            Renouveller
                        </button>
                ...
                <form class="d-flex flex-col align-items-center" @submit.prevent="onSubmit" :action="'/ticket/' + ticket.id + '/take'" method="post">
                    <input type="hidden" name="_token" :value="csrf">
        ...
        <pre class="overflow-hidden">{{ ticket.desc }}</pre>
        <div class="flex">
            <div v-for="(tag, index) in ticket.tags" :key="index" :style="'background-color: ' + tag.color" class="rounded-pill d-inline mr-1 mb-1 px-3 text-white">{{ tag.name }}</div>
        </div>
        <hr />
    </div>
</template>
